...
> MySQL支持的数据类型：
> [数值类型](https://dev.mysql.com/doc/refman/5.7/en/numeric-type-overview.html)，[日期和时间类型](https://dev.mysql.com/doc/refman/5.7/en/date-and-time-type-overview.html)，[字符串（字符和字节）的类型](https://dev.mysql.com/doc/refman/5.7/en/string-type-overview.html)，空间类型，以及 [JSON](https://dev.mysql.com/doc/refman/5.7/en/json.html) 数据类型。
